...
class EarlyStopping():
    ...
    def save_checkpoint(self, val_loss, model):
        ...
        # torch.save(model.state_dict(), self.filename+'.pkl')           
...
class Sep_Conv5(nn.Module):
    def __init__(self, in_ch, out_ch):
        super(Sep_Conv5, self).__init__()
        self.depth_conv = nn.Conv2d(
            in_channels=in_ch,
            out_channels=in_ch,
            kernel_size=5,
...
class Sep_Conv7(nn.Module):
    def __init__(self, in_ch, out_ch):
        super(Sep_Conv7, self).__init__()
        self.depth_conv = nn.Conv2d(
            in_channels=in_ch,
            out_channels=in_ch,
            kernel_size=7,
...
#%%
class InceptionBlock4(nn.Module):

    def __init__(self, in_channels,out_channels,padding,bias):
        super(InceptionBlock4, self).__init__()
        
        # mid=int(out_channels)
        # mid=int(mid/2)
        self.out=out_channels
        self.branch1x1 = nn.Conv2d(in_channels, out_channels, kernel_size=1)
        self.branch1x1_bn=torch.nn.BatchNorm2d(out_channels)
        
        self.branch3x3_1 = nn.Conv2d(in_channels, 8, kernel_size=1)
        self.branch3x3_bn=torch.nn.BatchNorm2d(8)        
        self.branch3x3_2 = Sep_Conv(8, out_channels)


        self.branch5x5_1 = nn.Conv2d(in_channels, 8, kernel_size=1)
        self.branch5x5_bn=torch.nn.BatchNorm2d(8)        
        self.branch5x5_2 = Sep_Conv(8, 16)
        self.branch5x5_3 = Sep_Conv(16, out_channels)   

        
        # self.branch5x5_1 = nn.Conv2d(in_channels, mid, kernel_size=1)
        # self.branch5x5_2 = Sep_Conv5(mid,out_channels)
        # self.branch3x3dbl_3 = Sep_Conv(8, out_channels)
        
        self.branch7x7_1 = nn.Conv2d(in_channels, 8, kernel_size=1)
        self.branch7x7_bn=torch.nn.BatchNorm2d(8)  
        self.branch7x7_2 = Sep_Conv(8, 16)
        self.branch7x7_3 = Sep_Conv(16, 32)  
        self.branch7x7_4 = Sep_Conv(32, out_channels)      
            #7x7
        # self.branch7x7_1 =nn.Conv2d(in_channels,out_channels*2, kernel_size=1) 
        # self.branch7x7 = Sep_Conv7(out_channels*2,out_channels)         
    ...
    def branch1(self, x):
        x=self.branch1x1(x)  
        x= self.branch1x1_bn(x)    
        x = F.relu(x,inplace=True)
        return x

    def branch3(self, x):
        x= self.branch3x3_1(x)
        x= self.branch3x3_bn(x)    
        x = F.relu(x,inplace=True)        
    ...
    def branch5(self, x):
        x=self.branch5x5_1(x)
        x= self.branch5x5_bn(x)    
        x = F.relu(x,inplace=True)          
        x = self.branch5x5_2(x)
        x = self.branch5x5_3(x)
        return x

    def branch7(self, x):
        x=self.branch7x7_1(x)
        x= self.branch7x7_bn(x) 
        x = self.branch7x7_2(x)   

        x = self.branch7x7_3(x)   
        x = self.branch7x7_4(x)   
        return x
    ...
    def forward(self, x):
        ...
        y3=checkpoint(self.branch5, y)
        y4=checkpoint(self.branch7, y)        

        outputs = [y1,y2, y3,y4]
        cnn1dOut=torch.tanh(torch.cat(outputs, 1))       
        # cnn1dOut=self.out1x1(torch.cat(outputs, 1))  #
        cnn1dOut=self.out1x1(cnn1dOut)  #
        # cnn1dOut=torch.tanh(cnn1dOut)    
        return cnn1dOut
#%%
...
#                              bias=self.bias)        
        ...
        self.conv = InceptionBlock4(in_channels=self.input_dim + self.hidden_dim,#卷積輸入的尺寸
                              out_channels=4 * self.hidden_dim,#因為lstmcell有四個門，隱藏層單元是rnn的四倍
                              padding=self.padding,
                              bias=self.bias)       
    def branch1(self, x):
...
class ConvLSTM(nn.Module):
    ...
    def forward(self, input_tensor, hidden_state=None):
        ...
        # input_tensor=F.max_pool3d(input_tensor,(1,2,2)) 
